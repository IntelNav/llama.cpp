...
ggml_tensor * llm_graph_context::build_inp_embd(ggml_tensor * tok_embd) const {
    ...
    //
    // IntelNav: a partial-model peer with `intelnav.has_embed=false`
    // never owns `tok_embd`. Normally the dual-path builder
    // constructs this branch unconditionally and lets the graph
    // select between tokens/embd at exec time, but building it
    // requires dereferencing `tok_embd` here. When it's absent, we
    // reuse the vector branch — the partial peer only ever feeds
    // `ubatch.embd` anyway, so the token branch is dead code.
    {
        auto & cur = inps[0];
